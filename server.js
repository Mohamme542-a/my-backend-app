...
app.get('/api/data', ...userChain, async (_q,r) => {
  try{
    const d=await fbGet('anasheed')||{};
    const items = Object.entries(d).map(([id,v])=>({id,...v}))
      .filter(v=>!v.hidden && !v.private)
...
app.get('/api/app-config', ...userChain, async (_q,r) => {
  try{ r.json((await fbGet('appConfig'))||{}); }
  catch(e){console.error('[cfg]',e.message);deny(r,502);}
});
...
app.get('/api/admin/app-config', ...adminChain, async (_q,r) => {
  try{ r.json((await fbGet('appConfig'))||{}); }
  catch(e){deny(r,502);}
});
app.post('/api/admin/app-config', ...adminChain, async (req,res) => {
  try{
    const b = req.body||{};
    const o = b.owner || {};
    const ol = o.links || {};
    const cfg = {
      appName:      String(b.appName||'').slice(0,60),
      brandName:    String(b.brandName||'').slice(0,60),
      homeTitle:    String(b.homeTitle||'').slice(0,80),
      homeSubtitle: String(b.homeSubtitle||'').slice(0,140),
      sectionsTitle:String(b.sectionsTitle||'').slice(0,60),
      favsTitle:    String(b.favsTitle||'').slice(0,60),
      downloadsTitle:String(b.downloadsTitle||'').slice(0,60),
      owner: {
        name:     String(o.name||'').slice(0,80),
        photoUrl: String(o.photoUrl||'').slice(0,800),
        bio:      String(o.bio||'').slice(0,1000),
        links: {
          facebook:  String(ol.facebook||'').slice(0,500),
          telegram:  String(ol.telegram||'').slice(0,500),
          instagram: String(ol.instagram||'').slice(0,500),
          youtube:   String(ol.youtube||'').slice(0,500),
          website:   String(ol.website||'').slice(0,500),
        }
      },
      updatedAt: Date.now(),
    };
    await fbPut('appConfig', cfg);
    res.json({ok:true});
  }catch(e){console.error('[cfg+]',e.message);deny(res,500);}
});

// CRUD: Sections
app.post('/api/admin/sections', ...adminChain, async (req,res) => {
  try{
    const { name, imageUrl, order, hidden, private: priv, featured, description, seoTitle, seoDesc } = req.body||{};
    ...
    const r = await fbPost('sections', {
      ...
      seoTitle: String(seoTitle||'').slice(0,120),
      seoDesc: String(seoDesc||'').slice(0,300),
      createdAt: Date.now(),
    });
    res.json({id:r.name});
...
app.put('/api/admin/sections/:id', ...adminChain, async (req,res) => {
  try{
    ...
    if (seoTitle!==undefined)    patch.seoTitle = String(seoTitle).slice(0,120);
    if (seoDesc!==undefined)     patch.seoDesc = String(seoDesc).slice(0,300);
...
// Reorder helper
async function applyReorder(path, items){
  if (!Array.isArray(items)) throw new Error('bad items');
  await Promise.all(items.slice(0,500).map((it,idx)=>{
    if (!it || !it.id) return null;
    const ord = Number.isFinite(it.order) ? it.order : idx;
    return fbPatch(`${path}/${encodeURIComponent(String(it.id))}`, { order: ord });
  }));
}
app.put('/api/admin/sections/reorder', ...adminChain, async (req,res) => {
  try{ await applyReorder('sections', (req.body||{}).items); res.json({ok:true}); }
  catch(e){ console.error('[sec-ord]',e.message); deny(res,500); }
});
app.put('/api/admin/side-menu/reorder', ...adminChain, async (req,res) => {
  try{ await applyReorder('sideMenu', (req.body||{}).items); res.json({ok:true}); }
  catch(e){ console.error('[sm-ord]',e.message); deny(res,500); }
});
app.put('/api/admin/anasheed/reorder', ...adminChain, async (req,res) => {
  try{ await applyReorder('anasheed', (req.body||{}).items); res.json({ok:true}); }
  catch(e){ console.error('[na-ord]',e.message); deny(res,500); }
});

// CRUD: Anasheed
app.post('/api/admin/anasheed', ...adminChain, async (req,res) => {
  try{
    const { title, artist, audioUrl, coverUrl, sectionId, tags, order, hidden } = req.body||{};
...
app.post('/api/admin/side-menu', ...adminChain, async (req,res) => {
  try{
    const { label, icon, action, order, sectionIds, hidden } = req.body||{};
    ...
    const r = await fbPost('sideMenu', {
      ...
      order: Number(order)||Date.now(),
      hidden: !!hidden,
      sectionIds: Array.isArray(sectionIds) ? sectionIds.slice(0,200).map(s=>String(s).slice(0,80)) : [],
    });
    res.json({id:r.name});
...
app.put('/api/admin/side-menu/:id', ...adminChain, async (req,res) => {
  try{
    ...
    if (order!==undefined)  patch.order = Number(order)||0;
    if (hidden!==undefined) patch.hidden = !!hidden;
    if (sectionIds!==undefined) patch.sectionIds = Array.isArray(sectionIds) ? sectionIds.slice(0,200).map(s=>String(s).slice(0,80)) : [];
